...
export default function ShiftPicker({ onComplete }) {
  return (
    ...
    >
      <p className="text-gray-500 text-lg mb-2">Good {greeting()}.</p>
      <h1 className="text-gray-900 dark:text-white text-3xl font-bold leading-tight mb-1">Starting a shift?</h1>
      <p className="text-gray-500 text-[15px] mb-8">Pick what you're working, nothing else to set up.</p>
      <div className="grid grid-cols-2 gap-3">
        {SHIFT_TYPES.map((s) => (
          <button
            key={s.key}
            onClick={() => onComplete({ shiftType: s.key })}
            className="py-4 rounded-xl text-base font-bold border bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800 active:scale-[0.98] transition-all min-h-[56px]"
          >
            {s.label}
          </button>
        ))}
