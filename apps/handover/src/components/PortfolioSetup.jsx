...
export default function PortfolioSetup({ initialRole, onComplete }) {
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-[15px] mb-10">One-time. This device is yours from here on.</p>

      <p className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-600 mb-3">Your role</p>
      <div className="flex flex-col gap-3">
        {ROLES.map((r) => (
          <button
            key={r.key}
            onClick={() => onComplete({ role: r.key })}
            className={`w-full py-4 rounded-2xl border text-left px-5 text-base font-bold active:scale-[0.98] transition-all min-h-[60px] ${
              initialRole === r.key
