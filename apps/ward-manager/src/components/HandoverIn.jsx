export default function HandoverIn({ beds, doctorName, onTakeOver, onClear }) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col px-6"
      ...
      <h1 className="text-white text-3xl font-bold leading-tight mb-2">Taking over</h1>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-800 mb-8">
        {bedList.map((bed, i) => {
          ...
          return (
            <div key={bed.id}
              className={`px-4 py-3.5 ${i > 0 ? "border-t border-gray-800/60" : ""}`}>
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-white font-bold text-sm">Bed {bed.bedNumber}</span>
      ...
      <button onClick={onTakeOver}
        className="w-full py-4 rounded-2xl bg-white text-gray-950 text-base font-bold mb-3 active:scale-95 transition-all">
        Take over ward
      </button>
      <button onClick={onClear}
        className="w-full py-4 rounded-2xl bg-transparent text-gray-600 text-base font-medium border border-gray-800 active:scale-95 transition-all">
