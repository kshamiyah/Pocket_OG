...
// backs every right/wrong verdict in the app — no source, no answer.
    ...
    for (const [i, beat] of c.beats.entries()) {
      ...
      if (beat.kind === "choice") {
        if (beat.fromNode) {
          ...
          const fcOpts = nodeOptions(beat.fromNode.fc, beat.fromNode.node);
          expect(fcOpts.length, `${where}: node has no options`).toBeGreaterThan(1);
          expect(beat.answer, `${where}: answer index out of range`).toBeLessThan(fcOpts.length);
          if (beat.options?.length) {
            expect(beat.options.length, `${where}: options must match flowchart node count`).toBe(fcOpts.length);
          }
        } else {
